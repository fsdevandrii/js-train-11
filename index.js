...
function customSet(arr) {
  ...
  let set = new Set(arr); // Створення множини з масиву
  for (let element of set) {
    // Перебір елементів множини
    if (typeof element === "number") {
      // Перевірка, чи є елемент числом
      set.delete(element); // Видалення числового елемента
    }
  }
  return set; // Повернення множини без числових значень
}
...
function clearSet(set) {
  ...
  if (set.size > 0) {
    // Перевірка, чи множина не порожня
...
function addElements(mySet, arr) {
  ...
  for (let element of arr) {
    ...
    mySet.add(element); // Додавання елемента до множини
  }
...
function filterAndAdd(mySet, arr) {
  ...
  for (let element of mySet) {
    ...
    if (typeof element === "number") {
      // Перевірка, чи є елемент числом
      mySet.delete(element); // Видалення числового елемента
    }
  }
  for (let element of arr) {
    // Перебір елементів масиву
...
function checkValueAndType(mySet, value) {
  ...
  if (mySet.has(value)) {
    // Перевірка, чи множина містить значення
...
function setToArray(mySet) {
  ...
  let array = [...mySet]; // Конвертація множини в масив

  // Фільтрація рядкових елементів
  array = array.filter((element) => typeof element === "string");

  // Сортування рядкових елементів в алфавітному порядку
  array.sort();

  return array; // Повернення обробленого масиву
}
...
function removeDuplicatesInPlace(arr) {
  ...
  while (i < arr.length) {
    if (set.has(arr[i])) {
      arr.splice(i, 1); // Видалення елемента за індексом, якщо він є дублікатом
    } else {
      set.add(arr[i]); // Додавання унікального елемента до множини
      i++; // Інкрементування лічильника
    }
  }

  return arr; // Повернення оновленого масиву без дублікатів
}
...
function areDisjoint(set1, set2) {
  ...
  for (let element of set1) {
    // Перебір елементів першої множини
    if (set2.has(element)) {
      // Перевірка, чи містить друга множина поточний елемент
...
function getDifference(set1, set2) {
  ...
  for (let element of set1) {
    // Перебір елементів першої множини
    if (!set2.has(element)) {
      // Перевірка, чи елемент не належить другій множині
      differenceSet.add(element); // Додавання елемента до differenceSet
    }
  }

  return differenceSet; // Повернення множини з елементами, що належать set1, але не належать set2
}
...
function getIntersection(arr1, arr2) {
  ...
  return intersectionSet; // Повертаємо множину зі спільними елементами
}
...
function iterateSet(set) {
  ...
  const keys = set.keys(); // Отримання ітератора ключів

  for (let key of keys) {
    // Ітерація по кожному ключу
    console.log(key); // Виведення ключа у консоль
  }

  const values = set.values(); // Отримання ітератора значень

  for (let value of values) {
    // Ітерація по кожному значенню
    console.log(value); // Виведення значення у консоль
  }

  const entries = set.entries(); // Отримання ітератора записів

  for (let entry of entries) {
    // Ітерація по кожному запису
    console.log(entry); // Виведення запису у консоль
  }
}
...
function sumNumbers(set) {
  ...
  set.forEach((element) => {
    // Перебір елементів множини за допомогою методу forEach
    if (typeof element === "number") {
      // Перевірка, чи є елемент числом
      sum += element; // Додавання числового елемента до суми
    }
  });

  return sum; // Повернення суми числових елементів у множині
}
